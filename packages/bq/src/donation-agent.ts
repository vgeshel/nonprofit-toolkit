...
import { z } from 'zod'
import { buildDonationsSchemaPrompt } from './donations-schema-prompt'
...
${buildDonationsSchemaPrompt(config.datasetCanon)}
...
export function createDonationAgent(
  ...
  return new ToolLoopAgent({
    ...
    tools: {
      query_bigquery: tool({
        description: [
          'Execute a read-only SELECT query against the donations events table and return the matching rows.',
          'Only SELECT is accepted: DDL and DML are rejected before execution and come back as an error string.',
          'A LIMIT is appended automatically if the query does not set one.',
          'At most 50 rows are returned regardless of LIMIT, while totalRows reports how many the query actually matched —',
          'if totalRows exceeds 50, aggregate in SQL rather than summing the rows you received.',
          'Returns {ok: true, rows, totalRows} or {ok: false, error}; a failed query is a result, not an exception,',
          'so read error and retry with corrected SQL.',
        ].join(' '),
